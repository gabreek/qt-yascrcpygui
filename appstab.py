...
class AppsTab(BaseGridTab):
    ...
    def redownload_all_icons(self):
        # Reset metadata for all apps in memory and triggers re-download
        for app in self.all_apps_data:
            pkg_name = app['key']
            if not app.get('is_launcher_shortcut', False):
                self.app_config.save_app_metadata(pkg_name, {'icon_fetch_failed': False})
                self.pending_icon_downloads[pkg_name] = app['name']
        
        if self.pending_icon_downloads:
            self._start_batch_icon_download()
        else:
            show_message_box(self, "Info", "No icons to download.", icon=QMessageBox.Information)
    ...
    def _save_metadata_async(self, pkg_name, data):
        # Simply save the metadata
        self.app_config.save_app_metadata(pkg_name, data)
